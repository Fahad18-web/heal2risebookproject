<?php
/**
 * Heal2Rise Book - User Notifications
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_all_read'])) {
    if (verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        $stmt = $db->prepare("UPDATE notifications SET is_read = 1 WHERE user_type = 'user' AND user_id = ?");
        $stmt->execute([$userId]);
...
if (isset($_GET['read'])) {
    ...
    $stmt = $db->prepare("UPDATE notifications SET is_read = 1 WHERE id = ? AND user_type = 'user' AND user_id = ?");
...
// Count unread
$stmt = $db->prepare("SELECT COUNT(*) FROM notifications WHERE user_type = 'user' AND user_id = ? AND is_read = 0");
$stmt->execute([$userId]);
            ...
            <div class="col-md-3 col-lg-2 mb-4">
                <div class="dashboard-sidebar">
                    ...
                    <nav class="nav flex-column">
                        ...
                        <a class="nav-link active" href="<?= url('/user/notifications.php') ?>">
