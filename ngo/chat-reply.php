<?php
/**
 * Heal2Rise Book - NGO Chat Reply
 * Interface for NGOs to reply to their assigned team member for a case.
 */
...
// Verify this case belongs to the current NGO and get team member
$stmt = $db->prepare("
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($msgText) || strlen($msgText) > 1000) {
            ...
            $result = sendChatMessage('ngo', $ngoId, 'team_member', $teamMemberId, $msgText, $caseId);
            if (!$result['success']) {
                setFlashMessage($result['error'], 'danger');
            }
        }
        
        redirect("/ngo/chat-reply.php?case_id={$caseId}");
        exit;
    }
}

// Mark messages as read when opening this page
$stmtRead = $db->prepare("UPDATE messages SET is_read = 1 WHERE receiver_type = 'ngo' AND receiver_id = ? AND sender_type = 'team_member' AND sender_id = ?");
$stmtRead->execute([$ngoId, $teamMemberId]);

// Load the full conversation thread
$messages = getChatThread('ngo', $ngoId, 'team_member', $teamMemberId, $caseId);
...
<div class="container py-4" style="max-width: 800px;">
    ...
    <div class="card shadow-sm mb-3 border-0">
        <div class="card-body d-flex align-items-center bg-primary text-white rounded">
            <a href="<?= url('/ngo/case-details.php?id=' . $caseId) ?>" class="text-white text-decoration-none me-3 fs-5">
        ...
        <div class="card-body p-4 overflow-auto bg-light" id="chat-box" style="height: 60vh; display: flex; flex-direction: column;">
            ...
            <?php else: ?>
                <?php foreach ($messages as $msg): 
                    $isMine = ($msg['sender_type'] === 'ngo' && $msg['sender_id'] == $ngoId);
                ?>
