<?php
/**
 * Heal2Rise Book - Admin NGOs Management
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($ngoId && in_array($action, ['approve', 'reject'])) {
...
if ($searchQuery) {
    $where[] = "(organization_name LIKE ? OR email LIKE ? OR city LIKE ?)";
...
// Count total
$stmt = $db->prepare("SELECT COUNT(*) FROM ngos $whereClause");
$stmt->execute($params);
$total = $stmt->fetchColumn();
$pagination = paginate($total, $perPage, $page);

// Get NGOs
$stmt = $db->prepare("SELECT * FROM ngos $whereClause ORDER BY created_at DESC LIMIT {$pagination['per_page']} OFFSET {$pagination['offset']}");
$stmt->execute($params);
$ngos = $stmt->fetchAll();
            ...
            <div class="col-md-9 col-lg-10">
                        ...
                        <form method="GET" class="row g-3 align-items-end">
                            <div class="col-md-5">
                                ...
                                <input type="text" class="form-control" name="search" 
                                       ...
                                       placeholder="Organization name, email, or city...">
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-2">
                                <a href="<?= url('/admin/ngos.php') ?>" class="btn btn-outline-secondary w-100">Reset</a>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <i class="bi bi-building-fill me-2"></i>NGOs (<?= $total ?>)
                    </div>
                    <div class="card-body">
                        <?php if (empty($ngos)): ?>
                            <p class="text-muted text-center py-4 mb-0">No NGOs found.</p>
                        <?php else: ?>
                                            ...
                                            <tr>
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        <img src="https://ui-avatars.com/api/?name=<?= urlencode($ngo['organization_name']) ?>&background=4A90A4&color=fff" 
                                                             class="rounded-circle me-2" width="40" height="40">
                                                        <div>
                                                            <strong><?= htmlspecialchars($ngo['organization_name']) ?></strong><br>
                                                            <small class="text-muted">Reg: <?= htmlspecialchars($ngo['registration_number']) ?></small>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <?= htmlspecialchars($ngo['contact_person']) ?><br>
                                                    <small class="text-muted"><?= htmlspecialchars($ngo['phone']) ?></small>
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    ?>
                                                        <span class="badge bg-info"><?= ucfirst(str_replace('_', ' ', $spec)) ?></span>
                                                    <?php endforeach; ?>
                                                    <?php if (count($specs) > 2): ?>
                                                        <span class="badge bg-secondary">+<?= count($specs) - 2 ?></span>
                                                    <?php endif; ?>
                                                </td>
                                                <td><?= $ngo['current_cases'] ?>/<?= $ngo['capacity'] ?></td>
                                                <td>
                                                ...
                                                <td>
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="<?= url('/admin/view-ngo.php?id=' . $ngo['id']) ?>" class="btn btn-outline-primary" title="View Details">
                                                            <i class="bi bi-eye"></i>
                                                        </a>
